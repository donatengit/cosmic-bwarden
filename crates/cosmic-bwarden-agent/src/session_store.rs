...
//! vault keys (`cosmic_bwarden_core::session_envelope`).
//!
...
use cosmic_bwarden_core::session_envelope::{self, MAX_ENVELOPE_BYTES};
use std::path::Path;
use std::sync::Arc;
...
pub fn save(
    ...
    save_to(
        &cosmic_bwarden_core::dirs::session_file(server, email),
        keys,
        server,
        email,
        refresh_token,
...
pub fn clear(server: &str, email: &str) -> anyhow::Result<()> {
    remove_if_present(&cosmic_bwarden_core::dirs::session_file(server, email))
}

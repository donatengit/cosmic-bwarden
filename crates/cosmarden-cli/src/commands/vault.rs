...
use cosmarden_core::db::{Field, Secret};
use cosmarden_core::protocol::{Action, EntryType as ProtocolEntryType, Response};
...
pub async fn handle_command(
    ...
    match &cli.command {
        ...
        Commands::Edit {
            ...
            for f in field {
                if let Some((k, v)) = f.split_once('=') {
                    entry.set_field(k, v, cosmarden_core::api::FieldType::Text);
                }
            }
            for f in secret_field {
                if let Some((k, v)) = f.split_once('=') {
                    entry.set_field(k, v, cosmarden_core::api::FieldType::Hidden);
                }
            }

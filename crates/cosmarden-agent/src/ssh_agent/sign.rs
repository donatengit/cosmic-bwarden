    ...
    loop {
        ...
        if !cache_has_pubkey(&g.ssh_identity_cache, &request.pubkey) {
            return Err(ssh_agent_lib::error::AgentError::other(
                cosmarden_core::error::Error::Other("no matching key found".to_string()),
            ));
        }
        ...
        .await
        {
            ...
            Ok(Err(_)) => {
                return Err(ssh_agent_lib::error::AgentError::other(
                    cosmarden_core::error::Error::Other("agent is locked".to_string()),
                ));
            }
            Ok(Ok(flag)) => *flag,
        };
...
fn timeout_err() -> Result<Signature, ssh_agent_lib::error::AgentError> {
    Err(ssh_agent_lib::error::AgentError::other(
        cosmarden_core::error::Error::Other("unlock wait timed out".to_string()),
    ))
}
...
fn sign_unlocked(
    ...
    for entry in &db.entries {
        if !matches!(entry.data, cosmarden_core::db::EntryData::SshKey { .. }) {
            continue;
        }
        let decrypted = entry.decrypt(keys, org_keys);
        if let cosmarden_core::db::EntryData::SshKey {
            private_key: Some(sk),
...
fn sign_with_key(
    ...
    match private_key.key_data() {
        ...
        other => Err(ssh_agent_lib::error::AgentError::other(
            cosmarden_core::error::Error::Other(format!("unsupported key type: {:?}", other)),
        )),
    }
}

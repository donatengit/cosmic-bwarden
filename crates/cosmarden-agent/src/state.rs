...
use cosmarden_core::protocol::{EntryType, SidebarEntry};
...
pub struct State {
    ...
    pub db: Option<cosmarden_core::db::Db>,
    ...
    pub subscribers: Vec<mpsc::UnboundedSender<cosmarden_core::protocol::Event>>,
...
impl State {
    ...
    pub fn rebuild_sidebar_cache(&mut self) {
        let new_cache = if let (Some(db), Some(keys)) = (&self.db, &self.keys) {
            ...
            for entry in &db.entries {
                ...
                let name = match cosmarden_core::vault::decrypt(
                    &entry.name,
                    effective_keys,
                    entry.key.as_deref(),
                ) {
                ...
                let (username, public_key, entry_type, hosts) = match &entry.data {
                    EntryData::Login { username, uris, .. } => {
                        let u = username.as_ref().and_then(|u| {
                            match cosmarden_core::vault::decrypt(u, effective_keys, entry.key.as_deref()) {
                                Ok(v) => Some(v),
                                Err(e) => {
                        ...
                        let mut hosts: Vec<String> = uris
                            .iter()
                            .filter(|u| {
                                u.match_type != Some(cosmarden_core::api::UriMatchType::Never)
                            })
                            .filter_map(|u| {
                                match cosmarden_core::vault::decrypt(
                                    &u.uri,
                                    effective_keys,
                                    entry.key.as_deref(),
                                ) {
                                    Ok(plain) => cosmarden_core::domain::host_from_uri(&plain),
                                    Err(e) => {
                                        log::warn!(
                        ...
                        if hosts.is_empty() {
                            ...
                            hosts.extend(cosmarden_core::domain::host_from_name(&name));
                        }
    ...
    pub fn broadcast(&mut self, event: cosmarden_core::protocol::Event) {
    ...
    pub fn lock(&mut self) {
        ...
        self.broadcast(cosmarden_core::protocol::Event::Locked);
    }
    ...
    pub fn request_unlock(&mut self) {
        if !self.unlock_requested_notified {
            ...
            let event = if self.tpm_configured {
                cosmarden_core::protocol::Event::PinRequested
            } else {
                cosmarden_core::protocol::Event::UnlockRequested
            };

...
use cosmarden_core::protocol::GeneratorHistoryEntry;
use cosmarden_core::{cipherstring::CipherString, locked};
...
fn load_or_create_key() -> Result<locked::Keys> {
    let path = cosmarden_core::dirs::generator_key_file();
...
fn load_stored_entries() -> Result<Vec<StoredEntry>> {
    let path = cosmarden_core::dirs::generator_history_file();
    match std::fs::read(&path) {
...
mod tests {
    ...
    // (crates/cosmarden-tests), which can isolate a per-test data dir.
    #[test]

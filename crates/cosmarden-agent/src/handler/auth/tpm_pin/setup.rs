...
pub async fn handle_setup_tpm_pin(
    ...
    #[cfg(feature = "tpm")]
    {
        ...
        let db = match cosmarden_core::db::Db::load(&config.server_name(), &email) {
            Ok(d) => d,
            Err(e) => {
                return Response::Error {
        ...
        let (vault_keys, _org_keys) = match cosmarden_core::vault::unlock_from_keys(
            &identity.keys,
            prot_key,
        ...
        let blob_path = cosmarden_core::dirs::tpm_blob_file(&server, &email);

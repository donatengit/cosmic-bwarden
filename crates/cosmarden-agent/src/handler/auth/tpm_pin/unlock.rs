...
pub async fn handle_unlock_with_pin(pin: String, state: &Arc<Mutex<State>>) -> Response {
    #[cfg(feature = "tpm")]
    {
        ...
        if !blob_path.exists() {
            ...
            return Response::Error {
                message: cosmarden_core::protocol::ERR_TPM_BLOB_MISSING.to_string(),
            };
        }
            ...
            Err(e) => {
                ...
                match crate::tpm::classify_unseal_failure(&e) {
                    crate::tpm::UnsealFailure::StateChanged => {
                        ...
                        return Response::Error {
                            message: cosmarden_core::protocol::ERR_TPM_STATE_CHANGED.to_string(),
                        };
                    }
                    _ => {
                        log::error!("TPM unseal failed: {:#}", e);
                        return Response::Error {
                            message: cosmarden_core::protocol::ERR_TPM_UNSEAL_FAILED.to_string(),
                        };
                    }
                }
            }
        ...
        let org_keys = match cosmarden_core::vault::decrypt_org_keys(
            &vault_keys,
        ...
        let has_token = {
            ...
            g.broadcast(cosmarden_core::protocol::Event::Unlocked);
            has_token
        };

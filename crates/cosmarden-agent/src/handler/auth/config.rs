...
pub async fn handle_version() -> Response {
    Response::Version {
        version: cosmarden_core::version().to_string(),
        protocol_version: cosmarden_core::PROTOCOL_VERSION.to_string(),
    }
}
...
pub async fn handle_get_config(state: &Arc<Mutex<State>>) -> Response {
    ...
    if state_guard.db.is_none() {
        if let Ok(config) = cosmarden_core::config::CosmardenConfig::load_legacy() {
            if let Some(email) = &config.email {
                if let Ok(mut db) = cosmarden_core::db::Db::load(&config.server_name(), email) {
    ...
    match cosmarden_core::config::CosmardenConfig::load_legacy() {
...
pub async fn handle_lock(state: &Arc<Mutex<State>>) -> Response {
    let email = cosmarden_core::config::CosmardenConfig::load_legacy()
        .ok()
        .and_then(|c| c.email)
...
pub async fn handle_logout(state: &Arc<Mutex<State>>) -> Response {
    ...
    if let Some(email) = &config.email {
        ...
        let db = cosmarden_core::db::Db::new();

    ...
    } = &entry.data.clone()
    {
        if let Some(pk) = private_key {
            entry.set_field(
                "private_key",
                pk.expose(),
                cosmarden_core::api::FieldType::Hidden,
            );
        }
        if let Some(pk) = public_key {
            entry.set_field("public_key", pk, cosmarden_core::api::FieldType::Text);
        }
    }
    let config = match cosmarden_core::config::CosmardenConfig::load_legacy() {
        Ok(c) => c,
    ...
    match res {
        Ok(()) => {
            state
                .lock()
                .await
                .broadcast(cosmarden_core::protocol::Event::VaultChanged);
            Response::Ack
        }
        Err(e) => {
            {

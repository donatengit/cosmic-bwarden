...
use cosmarden_core::api::UriMatchType;
use cosmarden_core::db::{EntryData, Secret};
use cosmarden_core::protocol::Response;
use std::sync::Arc;
use tokio::sync::Mutex;

use cosmarden_core::domain::{host_from_name, host_from_uri, hosts_match};
...
/// rules live in `cosmarden_core::domain` (exact / boundary-subdomain /
/// eTLD+1); free-text names never domain-match.
fn entry_matches_domain(name: &str, uris: &[cosmarden_core::db::Uri], domain: &str) -> bool {
...
pub async fn handle_check_login_match(
    ...
    for entry in &db.entries {
        ...
        if entry_password
            .as_ref()
            .is_some_and(|p| cosmarden_core::ct_eq(p.expose().as_bytes(), password.as_bytes()))
        {

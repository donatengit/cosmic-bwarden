...
//!
//! Run with:
//!   cargo test -p cosmarden-agent --features tpm -- --ignored
//!
...
#[ignore = "requires real TPM (/dev/tpmrm0 or /dev/tpm0); \
            run: cargo test -p cosmarden-agent --features tpm -- --ignored"]
...
#[tokio::test]
#[ignore = "requires real TPM; run: cargo test -p cosmarden-agent --features tpm -- --ignored"]
...
fn clear_directory_is_err() {
    let dir = std::env::temp_dir().join(format!("cosmarden_tpm_clear_dir_{}", std::process::id()));
...
fn clear_existing_file_is_ok() {
    let path =
        std::env::temp_dir().join(format!("cosmarden_tpm_clear_file_{}", std::process::id()));

...
fn is_unauthorized(e: &cosmarden_core::error::Error) -> bool {
...
where
    ...
    Fut: std::future::Future<Output = cosmarden_core::error::Result<T>>,
{
    let config =
        cosmarden_core::config::CosmardenConfig::load_legacy().map_err(|e| e.to_string())?;
        ...
        None => {
            ...
            match already {
                ...
                None => {
                    ...
                    .await
                    .map_err(|reason| {
                        format!("{}: {reason}", cosmarden_core::protocol::ERR_NO_SESSION)
                    })?;
                    let g = state.lock().await;
                    g.db.as_ref()
                        .and_then(|db| db.access_token.as_ref())

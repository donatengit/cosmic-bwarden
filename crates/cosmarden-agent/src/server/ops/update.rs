...
pub async fn update_entry_on_server(
    ...
    let entry_key = if let Some(ek) = &entry.key {
        match cosmarden_core::cipherstring::CipherString::new(ek) {
            Ok(ek_cipher) => match ek_cipher.decrypt_locked_symmetric(keys) {
                Ok(k) => Some(cosmarden_core::locked::Keys::new(k)),
    ...
    let (
        ...
        cosmarden_core::db::EntryData::Login {
            ...
            for uri in uris {
                let uri_enc = match cosmarden_core::cipherstring::CipherString::encrypt_symmetric(
                    crypt_keys,
                    uri.uri.as_bytes(),
                ) {
                    Ok(cs) => cs.to_string(),
        ...
        cosmarden_core::db::EntryData::SecureNote => (2, None, None, None, None, None, None, None),
        cosmarden_core::db::EntryData::Identity {
            ...
            let id = cosmarden_core::api::CipherIdentity {
                title: encrypt(title)?,
        ...
        cosmarden_core::db::EntryData::SshKey {
            ...
            (
                5,
                None,
                Some(cosmarden_core::api::CipherSshKey {

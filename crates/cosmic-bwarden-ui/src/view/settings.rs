...
use cosmic::widget::{button, container, list_column, settings as cosmic_settings, text};
use cosmic::Element;
    ...
    pub fn view_settings(&self) -> Element<'_, Message> {
        ...
        if is_editing {
            col = col.add(cosmic_settings::item(
                ...
                .on_input(Message::SettingsEmailChanged)
            ...
            col = col.add(cosmic_settings::item(
                ...
                cosmic::widget::text_input::text_input("Minutes", &self.settings_lock_timeout)
                    .on_input(Message::SettingsLockTimeoutChanged)
                    .width(Length::Fixed(300.0)),
            ));
            ...
            col = col.add(cosmic_settings::item(
                ...
                cosmic::widget::text_input::text_input("Count", &self.settings_popular_count)
                    .on_input(Message::SettingsPopularCountChanged)
                    .width(Length::Fixed(300.0)),
            ));

            col = col.add(cosmic_settings::item(
                "Top Entries Period (days)",
                cosmic::widget::text_input::text_input("Days", &self.settings_popular_days)
                    .on_input(Message::SettingsPopularDaysChanged)
                    .width(Length::Fixed(300.0)),
            ));
        } else {
            col = col.add(cosmic_settings::item(
                "Email",
                text::body(config.email.as_deref().unwrap_or("Not set")),
            ));
            col = col.add(cosmic_settings::item(
                "Server",
            ...
            col = col.add(cosmic_settings::item(
                "Auto-lock (minutes)",
                text::body(format!("{}", config.lock_timeout / 60)),
            ));
            col = col.add(cosmic_settings::item(
                "Top Entries Count",
                text::body(format!("{}", config.top_popular_count)),
            ));
            col = col.add(cosmic_settings::item(
                "Top Entries Period (days)",
                text::body(format!("{}", config.top_popular_days)),
            ));

            // Version (always shown, muted)
            col = col.add(cosmic_settings::item(
                "Version",
        ...
        let content = cosmic::widget::column::with_capacity(2)
            .spacing(20)
            .push(header)
            .push(col);

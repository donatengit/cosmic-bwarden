    ...
    pub fn view_settings(&self) -> Element<'_, Message> {
        ...
        if !is_editing {
            ...
            content = content.push(
                button::custom(
                    text::caption(format!("v{}", cosmarden_core::version())).class(muted_text()),
                )
                .on_press(Message::CopyToClipboard(
                    cosmarden_core::version().to_string(),
                ))
                .class(cosmic::theme::Button::Text)
                .padding([2, 0]),
            );
            content = content.push(
                button::custom(text::caption(cosmarden_core::HOMEPAGE).class(muted_text()))
                    .on_press(Message::CopyToClipboard(
                        cosmarden_core::HOMEPAGE.to_string(),
                    ))
                    .class(cosmic::theme::Button::Text)
                    .padding([2, 0]),
            );

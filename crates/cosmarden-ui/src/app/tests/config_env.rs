...
//! config (`~/.config/cosmarden/config.json`) when `COSMARDEN_CONFIG`
...
//! `cargo test -p cosmarden-ui` replaced a live config with an all-`null`
...
// COSMARDEN_CONFIG is process-global and tests run in parallel threads;
...
pub fn account_config() -> CosmardenConfig {
    CosmardenConfig {
...
/// Redirects `COSMARDEN_CONFIG` at a temporary file for the duration of a
...
impl ConfigFile {
    pub fn with(config: &CosmardenConfig) -> Self {
    ...
    pub fn read(&self) -> CosmardenConfig {

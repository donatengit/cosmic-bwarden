...
fn activate_filter_tab(app: &mut CosmardenApp, filter: Option<EntryType>) {
...
fn reveal_password_on_meta_entry_starts_get_password() {
    ...
    match vault_actions::on_demand_secret("Password", "1".into(), None) {
        cosmarden_core::protocol::Action::GetPassword { id, password } => {
            assert_eq!(id, "1");
...
fn edit_reprompt_sets_intent_and_submit_sends_get_entry() {
    ...
    match vault_actions::submit_reprompt_action(
        ...
        cosmarden_core::protocol::Action::GetEntry { id, password } => {
            assert_eq!(id, "1");
...
fn vault_split(app: &CosmardenApp) -> widget::pane_grid::Split {
    *app.vault_panes
        .layout()
        .splits()
        .next()

...
pub fn set_socket_override(path: PathBuf) {
    std::env::set_var("COSMARDEN_SOCKET", path);
}

pub fn set_ssh_socket_override(path: PathBuf) {
    std::env::set_var("COSMARDEN_SSH_SOCKET", path);
}
...
fn xdg() -> directories::ProjectDirs {
    directories::ProjectDirs::from("", "", &profile()).unwrap()
}

fn config_dir() -> std::path::PathBuf {
    xdg().config_dir().to_path_buf()
}

pub fn cache_dir() -> std::path::PathBuf {
    xdg().cache_dir().to_path_buf()
}

pub fn data_dir() -> std::path::PathBuf {
    xdg().data_dir().to_path_buf()
}

fn runtime_dir() -> std::path::PathBuf {
    let name = profile();
    xdg().runtime_dir().map_or_else(
        || {
            format!(
                "{}/{}-{}",

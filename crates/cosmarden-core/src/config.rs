...
impl Default for CosmardenConfig {
    fn default() -> Self {
        Self {
            email: None,
            base_url: None,
...
mod tests {
    ...
    fn save_legacy_forces_0600_file_and_0700_parent() {
        ...
        let prev = std::env::var_os("COSMARDEN_CONFIG");
        let dir = std::env::temp_dir().join(format!(
            "cosmarden-config-test-{}-{}",
            std::process::id(),
        ...
        let saved = CosmardenConfig::default().save_legacy();
        ...
        match prev {
            Some(v) => std::env::set_var("COSMARDEN_CONFIG", v),
            None => std::env::remove_var("COSMARDEN_CONFIG"),
        }

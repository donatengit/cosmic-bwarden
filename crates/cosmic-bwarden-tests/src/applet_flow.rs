...
async fn test_applet_unlock_after_lock() -> Result<()> {
    ...
    let res = client.send(Action::GetSidebarEntries { query: None, entry_type: None, only_pinned: true }).await?;
    assert!(matches!(res, Response::SidebarEntries { .. }), "Expected sidebar entries, got {:?}", res);
...
async fn test_applet_unlock_wrong_password() -> Result<()> {
    ...
    let res = client.send(Action::Unlock { password: "wrong-password".to_string() }).await?;
...
async fn test_ssh_public_key_in_sidebar_entries() -> Result<()> {
    ...
    let res = client.send(Action::GetSidebarEntries { query: None, entry_type: None, only_pinned: false }).await?;
    if let Response::SidebarEntries { entries } = res {
        let entry = entries.iter().find(|e| e.name == "My SSH Key").expect("ssh entry not found");
...
async fn test_get_password_for_secure_note() -> Result<()> {
    ...
    let id = if let Response::SidebarEntries { entries } = res {
        entries.iter().find(|e| e.name == "My Note").expect("note entry not found").id.clone()
    } else {
        anyhow::bail!("Expected sidebar entries");
    };

    let res = client.send(Action::GetPassword { id, password: None }).await?;
...
async fn test_get_password_reprompt_flow() -> Result<()> {
    ...
    let mut entry = if let Response::Entries { entries } = res {
        entries.into_iter().find(|e| e.name == "Sensitive").expect("entry not found")
    } else {
    ...
    let res = client.send(Action::GetPassword { id: entry.id.clone(), password: Some("wrong-password".to_string()) }).await?;
    assert!(matches!(res, Response::Error { .. }));

...
/// *inherited* `COSMIC_BWARDEN_PROFILE` from this process — and ~59 sibling
...
/// `cosmic-bwarden` profile, writing into the developer's real
...
/// old spawns inherited `COSMIC_BWARDEN_PROFILE` from this process, so which
...
async fn agent_spawn_writes_nothing_to_the_real_home() -> anyhow::Result<()> {
    ...
    for sub in ["cache", "data", "runtime"] {
        ...
        let has_profile_dir = std::fs::read_dir(&dir)?.flatten().any(|e| {
            e.file_name()
                .to_string_lossy()
                .starts_with("cosmic-bwarden-test-")
        });
        assert!(
            has_profile_dir,

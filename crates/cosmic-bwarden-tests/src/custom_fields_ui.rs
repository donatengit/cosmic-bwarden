...
async fn test_custom_fields_ui_simulation() -> Result<()> {
    ...
    let res = client.send(Action::GetEntries { query: Some("UIEntry".to_string()), entry_type: None }).await?;
    ...
    if let Some(f) = entry.fields.iter_mut().find(|f| f.name.as_deref() == Some("UIField")) {

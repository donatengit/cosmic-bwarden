...
async fn test_add_note_from_stdin() -> Result<()> {
    ...
    // Store a file's contents (e.g. `cat credentials.yaml | cosmarden-cli note add ... --stdin`)
...
async fn test_note_stdin_round_trip_byte_exact() -> Result<()> {
    ...
    // Store: `cat credentials.yaml | cosmarden-cli note add "AWS Creds" --stdin`
    let (success, _stdout, stderr) = run_cli_with_stdin(
        &env,
        &["note", "add", "AWS Creds", "--stdin"],
        CREDENTIALS_YAML,
    ...
    // Restore: `cosmarden-cli get "AWS Creds" --fields notes --show-secrets > credentials.yaml`

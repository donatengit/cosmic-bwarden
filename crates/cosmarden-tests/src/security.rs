...
async fn test_reprompt() -> Result<()> {
    ...
    let id = if let Response::Entries { entries } = res {
        ...
        if let cosmarden_core::db::EntryData::Login { password, .. } = &entries[0].data {
    ...
    entry.master_password_reprompt = cosmarden_core::api::CipherRepromptType::Password;
    client
...
async fn test_agent_events() -> Result<()> {
    ...
    // Connect to events
    use cosmarden_core::protocol::Event;

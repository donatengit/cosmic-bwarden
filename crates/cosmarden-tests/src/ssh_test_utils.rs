...
/// Asserts that the cosmarden ssh-agent socket either does (or does
...
pub fn assert_ssh_access(sock: &Path, port: u16, user: &str, expect_success: bool) -> Result<()> {
    ...
    if expect_success {
        ...
        let add_stdout = String::from_utf8_lossy(&add_output.stdout);
        if !add_stdout.contains("[cosmarden:locked]") {
            anyhow::bail!(
                "expected locked comment token [cosmarden:locked] in ssh-add -l, got {add_stdout:?}"
            );
        }

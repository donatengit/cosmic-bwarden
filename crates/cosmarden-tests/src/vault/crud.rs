...
async fn test_note_crud_lifecycle() -> Result<()> {
    ...
    let mut entry = if let Response::Entry { entry } = res {
        assert_eq!(
            entry.notes,
            Some(cosmarden_core::db::Secret::from(
...
async fn test_login_crud_lifecycle() -> Result<()> {
    ...
    if let Response::Entry { entry } = res {
        ...
        if let cosmarden_core::db::EntryData::Login { username, .. } = &entry.data {
...
async fn test_card_crud_lifecycle() -> Result<()> {
    ...
    let res = client
        ...
        .await?;
    if let Response::Entry { entry } = res {
        if let cosmarden_core::db::EntryData::Card {
            cardholder_name,
            number,
            ..
        } = &entry.data
...
async fn test_identity_crud_lifecycle() -> Result<()> {
    ...
    let res = client
        ...
        .await?;
    if let Response::Entry { entry } = res {
        if let cosmarden_core::db::EntryData::Identity {
            first_name,
            last_name,
            email,
            ..

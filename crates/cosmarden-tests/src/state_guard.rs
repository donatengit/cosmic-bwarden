...
//! from `cosmarden_core::dirs::`, but those functions read process-global
//! env — and `COSMARDEN_PROFILE` in this test binary is whatever the last
//! sibling test's `set_var` left behind, or unset. Unset resolves to the
//! *live* `cosmarden` profile, so such a `Drop` would erase the
...
/// A listing of every `cosmarden*` entry under the real roots.
///
    ...
    pub fn take() -> Self {
        ...
        for root in real_roots() {
            ...
            for entry in dir.flatten() {
                if entry.file_name().to_string_lossy().starts_with("cosmarden") {
    ...
    pub fn assert_nothing_added(&self, context: &str) {
        ...
        assert!(
            ...
            "{context} leaked state into the real home — these paths did not \
             ...
             The spawn is missing an explicit COSMARDEN_PROFILE, HOME, or \
...
/// Sets `COSMARDEN_PROFILE` for the duration of a test and restores the
...
/// before any of them) under the *live* `cosmarden` profile.
///
...
impl ProfileEnv {
    pub fn set(profile: &str) -> Self {
        let previous = std::env::var_os("COSMARDEN_PROFILE");
...
mod tests {
    ...
    /// The snapshot must actually notice a new `cosmarden*` dir. Written
    ...
    fn snapshot_detects_a_new_profile_dir() {
        ...
        let planted = cache.join("cosmarden-test-state-guard");

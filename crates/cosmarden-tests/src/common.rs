...
pub struct TestEnv {
    ...
    /// Listing of `cosmarden*` entries in the developer's real home,
...
impl TestEnv {
    ...
    pub fn start_agent_with_env(&self, extra_env: &[(&str, &str)]) -> Result<std::process::Child> {
        ...
        cmd.arg("--socket")
            ...
            .env("COSMARDEN_PROFILE", &self.profile)
    ...
    pub fn cli_path(&self) -> PathBuf {
        ...
        cli_path.pop();
        cli_path.pop();
        cli_path.push("target/debug/cosmarden");
        cli_path
    }
    ...
    fn apply_cli_env(&self, cmd: &mut Command) {
        cmd.env("COSMARDEN_PROFILE", &self.profile)
            .env("XDG_CONFIG_HOME", &self.config_home)
            .env("XDG_CACHE_HOME", &self.cache_home)
            .env("XDG_DATA_HOME", &self.data_home)
            .env("XDG_RUNTIME_DIR", &self.runtime_home);
...
/// started by this suite also carries the `com.enikeev.cosmarden.e2e`
...
pub async fn setup_env_no_agent() -> Result<TestEnv> {
    ...
    let node = GenericImage::new("vaultwarden/server", "latest")
        ...
        .with_label("com.enikeev.cosmarden.e2e", "true");
...
pub async fn register_user(url: &str, email: &str, password: &str) -> Result<()> {
    ...
    use cosmarden_core::api::KdfType;
    ...
    use cosmarden_core::cipherstring::CipherString;

    let protected_key = CipherString::encrypt_symmetric(&identity.keys, identity.keys.data())?;

    let register_payload = serde_json::json!({
        "email": email,
        "masterPasswordHash": cosmarden_core::base64::encode(identity.master_password_hash.hash()),
        "masterPasswordHint": "",
...
pub async fn logout_login_cycle(
    ...
    let res = client
        ...
        .await?;
    if let Response::Error { message } = res {
        let no_session = message.contains(cosmarden_core::protocol::ERR_NO_SESSION);

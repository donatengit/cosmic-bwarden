...
//!   - The `cosmarden-agent-tpm` binary (compiled with `--features tpm`).
...
//!   - `target/debug/cosmarden-agent-tpm` built with
...
/// Can be overridden with `COSMARDEN_TPM_AGENT` env var.
pub fn tpm_agent_path() -> Option<PathBuf> {
    // Allow CI / dev to point at a custom build
    if let Ok(p) = env::var("COSMARDEN_TPM_AGENT") {
        let pb = PathBuf::from(p);
        if pb.exists() {
            return Some(pb);
        }
        eprintln!("[tpm-smoke] COSMARDEN_TPM_AGENT={} not found", pb.display());
        return None;
    }

    let mut p = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    p.pop(); // cosmarden-tests
    p.pop(); // crates
    p.push("target/debug/cosmarden-agent-tpm");
    ...
    if p.exists() {
        ...
        eprintln!(
            "[tpm-smoke] {} not found — build with: \
             cargo build -p cosmarden-agent --features tpm\n\
             (produces target/debug/cosmarden-agent-tpm)",
            p.display()
        );
        None
    }

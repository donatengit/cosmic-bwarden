#!/usr/bin/env python3
"""Register the Cosmarden native messaging host for Firefox.
...
DESCRIPTION = "Cosmarden Native Messaging Host"
...
def register_firefox(agent_path: str, home: str) -> None:
    ...
    wrapper_path = os.path.join(manifest_dir, "cosmarden-browser-host.sh")
...
if __name__ == "__main__":
    ...
    default_agent = os.path.join(project_root, "target", "debug", "cosmarden-agent")

    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument(
        "--agent-path",
        default=default_agent,
        help="Path to cosmarden-agent binary (default: target/debug/)",
    )
    parser.add_argument(
        "--home",
    ...
    if not os.path.exists(args.agent_path):
        ...
        if args.agent_path == default_agent:
            print("Build it first: cargo build -p cosmarden-agent")
        sys.exit(1)
